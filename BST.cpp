...
//  an empty tree.
//=================================================
template <typename D, typename K> BST<D, K>::BST(void) { root = nullptr; }
...
//=================================================
template <typename D, typename K> BST<D, K>::~BST(void) {
  if (root == nullptr) {
    return;
  }
...
//  true or false
//=================================================
template <typename D, typename K> bool BST<D, K>::empty() {
  return root == nullptr;
}
...
//
//=================================================
template <typename D, typename K> void BST<D, K>::insert(D d, K k) {
  // From page 321 of textbook
  Node<D, K> *z = new Node<D, K>{.left = nullptr,
                                 .right = nullptr,
                                 .parent = nullptr,
                                 .data = d,
                                 .key = k};
...
//=================================================
template <typename D, typename K> D BST<D, K>::get(K k) {
  auto found = iterative_tree_search(root, k);
  if (found == nullptr) {
    std::cout << "Warning: default data value returned for get call because "
                 "passed in key doesn't exist"
              << std::endl;
...
//  K key
//=================================================
template <typename D, typename K> void BST<D, K>::remove(K k) {
...
//=================================================
template <typename D, typename K> Node<D, K> *max(Node<D, K> *x) {
...
//=================================================
template <typename D, typename K> D BST<D, K>::max_data() {
  if (empty()) {
    return default_data_value<D>();
  }
...
//=================================================
template <typename D, typename K> K BST<D, K>::max_key() {
  if (empty()) {
    std::cout
        << "Warning: default key of 0 returned for max_key call on empty bst"
        << std::endl;
    return 0;
  }
...
//=================================================
// min
// Returns the minimum node in the tree or nil if x is nil. From page 318 in the
// book.
//
// RETURN VALUE:
//  The minimum node in the tree, or nil x is nil.
//=================================================
template <typename D, typename K> Node<D, K> *BST<D, K>::min(Node<D, K> *x) {
  if (x == nullptr) {
    return nullptr;
  }
...
//=================================================
template <typename D, typename K> D BST<D, K>::min_data() {
  if (empty()) {
    return default_data_value<D>();
  }
...
//=================================================
template <typename D, typename K> K BST<D, K>::min_key() {
  if (empty()) {
    std::cout
        << "Warning: default key of 0 returned for min_key call on empty bst"
        << std::endl;
    return 0;
  }
...
//=================================================
template <typename D, typename K> K BST<D, K>::successor(K k) {
  ...
  if (x->right != nullptr) {
    ...
    if (y == nullptr) {
      std::cout << "Warning: No successor found, default value of 0 returned"
                << std::endl;
      return 0;
    }
    return y->key;
...
//  A string of keys.
//=================================================
template <typename D, typename K> string BST<D, K>::to_string() const {
  if (root == nullptr) {
    return "";
  }
...
template <typename D, typename K> string BST<D, K>::in_order(Node<D, K> *x) {
  stringstream s;
...
//=================================================
template <typename D, typename K> string BST<D, K>::in_order() {
  if (root == nullptr) {
    return "";
  }
  ...
  in_order_str =
      in_order_str.substr(0, in_order_str.size() - 1); // Remove trailing space
  return in_order_str;
}

//=================================================
// delete_BST(Node<D, K>*)
// Convenience function to delete a single node with delete_BST(Node<D, K>*,
// Node<D, K>*)
//=================================================
template <typename D, typename K> static void delete_BST(Node<D, K> *node) {
...
//=================================================
template <typename K> bool in_range(K low, K high, K val) {
...
//  K low, K high
//=================================================
template <typename D, typename K> void BST<D, K>::trim(K low, K high) {
  if (high < low) {
